...
class MySQLWorkspaceConversationService(_MySQLBase):
    """MySQL-backed workspace conversation metadata and messages."""
        ...
        with self._tx() as cur:
            cur.execute(
                """INSERT INTO runtime_conversation
                   ...
                   VALUES (%s,'local','web',%s,'active',NULL,%s,%s)""",
                (conversation_id, self.clean_title(title), now, now))
    ...
    def messages(self, conversation_id: str, *, offset: int = 0,
        ...
        with self._tx() as cur:
            cur.execute(
                """SELECT COUNT(*) AS total FROM runtime_message
                   WHERE conversation_id=%s AND role IN ('user','assistant') AND archived_at IS NULL""",
                (conversation_id,),
            )
            total = int(cur.fetchone()["total"])
            cur.execute(
                """SELECT role,content_json,created_at FROM runtime_message
                   WHERE conversation_id=%s AND role IN ('user','assistant') AND archived_at IS NULL
        ...
        messages = []
        for row in rows:
            payload = json.loads(row["content_json"]) if isinstance(row["content_json"], str) else row["content_json"]
            if isinstance(payload, dict) and isinstance(payload.get("_nanoclaw_message"), dict):
                message = payload["_nanoclaw_message"]
                content = message.get("content")
            else:
                content = payload
            messages.append({"role": row["role"], "content": content, "timestamp": _iso(row["created_at"])})
...
def conversation_backend() -> str:
    ...
    value = os.environ.get(
        "NANOCLAW_CONVERSATION_BACKEND", "mysql" if profile == "production" else "local"
    ).strip().lower()
