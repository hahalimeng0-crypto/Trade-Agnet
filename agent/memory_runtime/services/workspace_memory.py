...
from ..stores.sqlite import content_hash, utcnow
...
class WorkspaceMemoryService:
    ...
    def create_candidate(
        ...
        try:
            ...
            if not isinstance(exc, sqlite3.IntegrityError) and exc.__class__.__name__ != "IntegrityError":
                raise
    ...
    async def run_once(self, actor: ActorContext, scope: MemoryScope) -> int:
        ...
        with self.store._lock, self.store.connection:
            for raw in suggestions[:100]:
                ...
                count += max(0, int(result.rowcount))
        return count
