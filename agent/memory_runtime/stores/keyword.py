...
from ..models import ActorContext, MemoryItem, MemoryScope
...
class AuthoritativeKeywordMemoryIndex:
    """Lexical candidate adapter backed directly by authoritative MySQL rows.

    It deliberately persists no second long-term copy.  ``upsert`` and ``delete``
    are no-ops because the MySQL transaction has already changed the searchable
    source of truth before an outbox event is processed.
    """

    def __init__(self, authoritative_store):
        self.store = authoritative_store
    ...
    @staticmethod
    def _actor(scope: MemoryScope) -> ActorContext:
        if scope.realm in {"customer_private", "customer_conversation"}:
            return ActorContext(
                "customer", scope.account_id or "", scope.tenant_id,
                frozenset(), True,
            )
        return ActorContext(
            "workspace_operator", scope.subject_id or "memory-index",
            scope.tenant_id,
            frozenset({"workspace_memory_reader"}), True,
        )

    def search(self, scope: MemoryScope, query: str, limit: int) -> list[IndexHit]:
        hits = self.store.search(self._actor(scope), scope, query, limit)
        return [IndexHit(hit.item.memory_id, hit.score) for hit in hits]

    @property
    def index_version(self) -> int:
        # MySQL is already the current version; this value only labels diagnostics.
        return 1
