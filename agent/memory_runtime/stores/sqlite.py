...
class WorkspaceSQLiteMemoryStore:
    ...
    def create_candidate(self, actor, item):
        ...
        with self._lock, self.connection:
            self.connection.execute(
                """INSERT INTO workspace_memory_item
                   (memory_id,tenant_id,subject_id,project_id,memory_type,purpose,content,
                    summary,source_refs_json,status,confidence,importance,sensitivity,version,
                    supersedes,content_hash,created_at,updated_at,valid_from,expires_at)
                   VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
