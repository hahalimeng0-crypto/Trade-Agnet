...
def deterministic_extract_rfq_fields(document: str, source_context: dict) -> dict:
    ...
    for match in quantity_pattern.finditer(source):
        ...
        # Decimal version numbers such as USB 3.0 belong to the specification;
        # only a sentence-ending full stop should terminate the field.
        spec_match = re.match(r"\s*,\s*((?:\.(?=\d)|[^.;\n])+)", sentence_tail)
    ...
    deadline_patterns = (
        rf"(?i)\b(?:before|by)\s+\d{{1,2}}\s+(?:{_MONTHS})(?:\s+\d{{4}})?\b",
        rf"(?i)\bbetween\s+\d{{1,2}}\s+and\s+\d{{1,2}}\s+(?:{_MONTHS})\s+\d{{4}}\b",
    ...
    # Count only actual calendar dates. The old broad expression treated product
    # specifications such as "1 meter" as a second date and rejected valid RFQs.
    date_mentions = re.findall(
        rf"(?i)\b\d{{1,2}}\s+(?:{_MONTHS})(?:\s+\d{{4}})?\b", source
    )
