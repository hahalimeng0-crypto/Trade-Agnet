...
class EmailQuoteWorkflowService:
    ...
    def regenerate_approval(self, quote_id: int, *, reviewer: str) -> dict:
        """Create a fresh approval bound to the current immutable quote version."""
        if not reviewer.strip():
            raise EmailQuoteWorkflowError("email_approver_not_configured", 503)
        ...
        if quote is None:
            raise EmailQuoteWorkflowError("email_quote_not_found", 404)
        if quote["status"] in {"approved", "sent"}:
            raise EmailQuoteWorkflowError("email_approval_regeneration_not_allowed", 409)
        versions = json.loads(quote["version_data"] or "[]")
        version_number = int(quote["current_version"] or 0)
        version = next(
            (item for item in versions if int(item.get("version", 0)) == version_number), None
        )
        if version is None:
            raise EmailQuoteWorkflowError("email_approval_regeneration_required", 409)
        content_hash = canonical_quote_hash(version)
        existing = self.connection.execute("""SELECT * FROM approval_records
          WHERE quote_id=? AND version=? AND status='pending' AND content_hash=?
          ORDER BY id DESC LIMIT 1""", (int(quote_id), version_number, content_hash)).fetchone()
        if existing:
            workflow = self.connection.execute(
                "SELECT * FROM ops_email_quote_workflow WHERE quote_id=?", (int(quote_id),)
            ).fetchone()
            if workflow and int(workflow["approval_key"] or 0) != int(existing["id"]):
                now = _now()
                self.connection.execute("""UPDATE ops_email_quote_workflow
                  SET quote_version=?,approval_key=?,status='pending_approval',error_code=NULL,updated_at=?
                  WHERE quote_id=?""", (version_number, int(existing["id"]), now, int(quote_id)))
                self.connection.commit()
                workflow = self.connection.execute(
                    "SELECT * FROM ops_email_quote_workflow WHERE quote_id=?", (int(quote_id),)
                ).fetchone()
            if workflow:
                return self._work_item(dict(workflow))
            return {
                "email_id": None, "quote_id": int(quote_id), "quote_version": version_number,
                "approval_key": int(existing["id"]), "work_status": "pending_approval",
                "error_code": None, "content_hash": content_hash,
                "total_usd": version.get("total_usd"),
            }

        now = _now()
        try:
            self.connection.execute("BEGIN IMMEDIATE")
            self.connection.execute("""UPDATE approval_records
              SET status='superseded',reviewer=?,comment=?,decided_at=?
              WHERE quote_id=? AND status='pending'""", (
                reviewer.strip(), "Superseded by regenerated approval", now, int(quote_id)
            ))
            cursor = self.connection.execute("""INSERT INTO approval_records(
              quote_id,version,status,content_hash,created_at)
              VALUES(?,?,'pending',?,?)""", (
                int(quote_id), version_number, content_hash, now
            ))
            approval_key = int(cursor.lastrowid)
            self.connection.execute(
                "UPDATE quotes SET status='pending_approval' WHERE id=?", (int(quote_id),)
            )
            self.connection.execute("""UPDATE ops_email_quote_workflow
              SET quote_version=?,approval_key=?,status='pending_approval',error_code=NULL,updated_at=?
              WHERE quote_id=?""", (version_number, approval_key, now, int(quote_id)))
            self.connection.commit()
        except Exception:
            self.connection.rollback()
            raise
        ...
        return {
            "email_id": None, "quote_id": int(quote_id), "quote_version": version_number,
            "approval_key": approval_key, "work_status": "pending_approval",
            "error_code": None, "content_hash": content_hash,
            "total_usd": version.get("total_usd"),
        }
