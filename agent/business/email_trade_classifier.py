...
def classify_trade_rfq_email(envelope: EmailEnvelope, *, mailbox_address: str = "",
    ...
    auto_submitted = envelope.auto_submitted.strip().lower()
    precedence = envelope.precedence.strip().lower()
    if ((auto_submitted and auto_submitted != "no")
            or precedence in {"bulk", "junk", "list"}
            or sender.startswith(("mailer-daemon@", "postmaster@"))):
        return TradeEmailClassification(False, "email_trade_automated_message")
