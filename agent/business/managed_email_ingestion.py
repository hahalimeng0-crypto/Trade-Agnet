...
class ManagedEmailIngestionRuntime:
    def __init__(self, config: EmailIngestionConfig, *,
                 ...
                 clarification_repository: EmailClarificationRepository | None = None,
        ...
        if config.clarification_auto_reply_enabled and self.clarification_repository is None:
            self.clarification_repository = EmailClarificationRepository(
                self.email_repository.connection
            )
    ...
    async def poll_due(self) -> list[dict]:
        ...
        for account in self.eligible_accounts():
            ...
            try:
                ...
                if self.config.clarification_auto_reply_enabled:
                    for message in messages:
                        if message.get("status") != "needs_review":
                            continue
                        row = self.email_repository.get(int(message["email_id"]))
                        try:
                            extraction = json.loads(row["extraction_json"] or "{}")
                            delivery = self.clarification_repository.queue_for_inbound(
                                int(message["email_id"]), extraction,
                            )
                            message["clarification_status"] = delivery["status"]
                        except EmailClarificationError as exc:
                            message["clarification_status"] = "not_queued"
                            message["clarification_error_code"] = str(exc)
