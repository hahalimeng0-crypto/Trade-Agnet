...
class CustomerIdentityService:
    ...
    def _issue(self, account_id: str, locale: str, now_dt: datetime) -> IssuedCustomerSession:
        ...
        if self.jwt_codec is not None:
            access_token = self.jwt_codec.issue(Principal(
                account_id, self.tenant_id, account_id, frozenset({"customer"}),
            ))
            access_expires_in = self.jwt_codec.lifetime_seconds
        return IssuedCustomerSession(
            customer, token, csrf, idle, absolute, access_token, access_expires_in,
        )
    ...
    def resolve_access(self, token: str | None):
        if not token or self.jwt_codec is None:
            return None
        try:
            principal = self.jwt_codec.decode(token)
        except JWTError:
            return None
        if principal.roles != frozenset({"customer"}) or principal.tenant_id != self.tenant_id:
            return None
        return self.repository.resolve_account(principal.tenant_id, principal.user_id)

    def require_request(
        self, cookie_token: str | None, authorization: str | None,
        csrf_token: str | None = None,
    ):
        scheme, separator, supplied = (authorization or "").partition(" ")
        if separator and scheme.casefold() == "bearer" and supplied:
            customer = self.resolve_access(supplied)
            if customer is None:
                raise CustomerIdentityError(CUSTOMER_AUTH_REQUIRED)
            # Bearer credentials are not ambient browser cookies, so CSRF does not apply.
            return customer
        customer = self.require(cookie_token)
        if csrf_token is not None:
            self.require_csrf(customer, csrf_token)
        return customer
