...
class ToolRegistry:
    ...
    def registered_tools(self) -> list[Tool]:
        """Return the tools owned by this Agent without changing inheritance policy."""
        return list(self._tools.values())

    def inheritable_tools(self) -> list[Tool]:
        """Return tools explicitly safe to copy into an untrusted subagent."""
        return [
            tool for tool in self._tools.values()
            if tool.name != "spawn_subagent"
            and getattr(tool, "allow_subagent_inheritance", True)
        ]
