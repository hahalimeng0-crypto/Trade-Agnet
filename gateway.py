...
from llm_routing import LLMRouteClassifier, RouteError
...
class Gateway:
    ...
    def __init__(
        ...
        *,
        ...
        route_classifier: LLMRouteClassifier | None = None,
        routed_agent_factory: Callable[[str, str], AgentLoop] | None = None,
    ) -> None:
        """
        Args:
            bus: 消息总线实例
        ...
        self.routed_agent_factory = routed_agent_factory
    ...
    async def _handle_inbound(self, inbound_msg: InboundMessage) -> None:
        ...
        if event == "conversation_deleted":
            removed_agents = [
                self._agents.pop(key) for key in list(self._agents)
                if key == session_key or key.endswith(f":{session_key}")
            ]
            for removed_agent in removed_agents:
                if hasattr(removed_agent, "clear_peer_history"):
        ...
        try:
            ...
            async with self._semaphore, lock:
                ...
                if self.route_classifier is not None:
                    try:
                        principal = Principal.from_trusted_dict(raw.get("_principal"))
                    except ValueError as exc:
                        raise RouteError("authentication_required") from exc
                    decision = await self.route_classifier.route(
                        principal, inbound_msg.content,
                    )
                    selected_agent = decision.agent.value
                cache_key = (
                    f"{selected_agent}:{session_key}" if selected_agent else session_key
                )
                agent = None if shared else self._agents.get(cache_key)
                if agent is None:
                    if selected_agent is None:
                        agent = self.agent_factory(session_key)
                    else:
                        if self.routed_agent_factory is None:
                            raise RuntimeError("routed_agent_factory_not_configured")
                        agent = self.routed_agent_factory(session_key, selected_agent)
                    if not shared:
                ...
                if hasattr(agent, "set_request_context"):
                    ...
                    if selected_agent is not None:
                        # Forward only the Principal that was decoded and accepted above.
                        # Never copy a model/user supplied identity into an Agent tool context.
                        trusted_context["_principal"] = principal.to_trusted_dict()
                    if (inbound_msg.channel == "customer_portal"
                            and "tenant_id" in raw and "account_id" in raw):
                        if principal is not None:
                            if (str(raw["tenant_id"]) != principal.tenant_id
                                    or str(raw["account_id"]) != principal.user_id):
                                raise RouteError("authenticated_scope_mismatch")
                        ...
                        trusted_context.update({
                            key: raw[key] for key in ("conversation_id", "request_id")
                            if key in raw
                        })
            ...
            await self.bus.publish_outbound(OutboundMessage(
                channel=inbound_msg.channel, chat_id=inbound_msg.chat_id,
                content="", conversation_id=conversation_id, request_id=request_id,
                event_type="error", error_code=exc.code,
            ))
