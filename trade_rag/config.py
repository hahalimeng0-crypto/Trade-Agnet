...
def load_rag_vector_config() -> RagVectorConfig:
    ...
    backend = os.environ.get(
        "RAG_VECTOR_BACKEND", "milvus" if profile == "production" else "memory"
    ).strip().lower()
    config = RagVectorConfig(
        ...
        milvus_uri=_first_env("RAG_MILVUS_URI", "NANOCLAW_MILVUS_URI").strip(),
        milvus_token=_first_env("RAG_MILVUS_TOKEN", "NANOCLAW_MILVUS_TOKEN"),
        milvus_database=_first_env(
            "RAG_MILVUS_DATABASE", "NANOCLAW_MILVUS_DATABASE", default="nanoclaw_vector_docker"
        ).strip(),
...
def load_rag_keyword_config() -> RagKeywordConfig:
    ...
    config = RagKeywordConfig(
        backend=os.environ.get(
            "RAG_KEYWORD_BACKEND", "elasticsearch" if profile == "production" else "memory"
        ).strip().lower(),
        url=_first_env("RAG_ELASTICSEARCH_URL", "NANOCLAW_ELASTICSEARCH_URL").strip().rstrip("/"),
        username=_first_env(
            "RAG_ELASTICSEARCH_USERNAME", "NANOCLAW_ELASTICSEARCH_USERNAME"
        ).strip(),
        password=_first_env("RAG_ELASTICSEARCH_PASSWORD", "NANOCLAW_ELASTICSEARCH_PASSWORD"),
        index=_first_env(
            "RAG_ELASTICSEARCH_INDEX", "NANOCLAW_ELASTICSEARCH_INDEX",
            default="trade_knowledge_child_v1",
        ).strip().lower(),
