...
from channels.access_auth_api import ACCESS_COOKIE
...
def create_customer_auth_router(service: CustomerIdentityService) -> APIRouter:
    ...
    async def login(
        ...
        if issued.access_token:
            response.set_cookie(
                ACCESS_COOKIE, issued.access_token, httponly=True, samesite="strict",
                secure=request.url.scheme == "https", max_age=issued.access_expires_in,
            )
        result = {
            "authenticated": True,
